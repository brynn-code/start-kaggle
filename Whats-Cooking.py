...
dataArr_test = X[:-len(dataArr_test)]
print(len(dataArr))
print(len(dataArr_test))
dataArr = np.array(dataArr).reshape(-1, comp_num)
dataArr_test = np.array(dataArr_test).reshape(-1, comp_num)

b,alphas = smoP(dataArr, labelArr, 200, 0.01, 10000, ('rbf', 1.3)) #通过SMO算法得到b和alpha
datMat=mat(dataArr)
...
print("test size is %d %d" %(m, n))
test_pred = []
for i in range(m): 
    kernelEval = kernelTrans(sVs,datMat_test[i,:],('rbf', 1.3))
    predict=kernelEval.T * multiply(labelSV,alphas[svInd]) + b
    test_pred.append(test_data[i].id, classes[(int)(predict)])
